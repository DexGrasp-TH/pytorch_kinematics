...
JOINT_TYPE_MAP = {mujoco.mjtJoint.mjJNT_HINGE: "revolute", mujoco.mjtJoint.mjJNT_SLIDE: "prismatic"}
...
def body_to_geoms(m: mujoco.MjModel, body: MjModelBodyViews):
    ...
    for geom_id in range(m.ngeom):
        ...
        if geom.bodyid == body.id:
            visuals.append(
                frame.Visual(
                    offset=tf.Transform3d(rot=geom.quat, pos=geom.pos), geom_type=geom.type, geom_param=geom.size
                )
            )
    return visuals
...
def _build_chain_recurse(m, parent_frame, parent_body):
    ...
    for body_id in range(m.nbody):
        ...
        if body.parentid == parent_body.id and body_id != parent_body.id:
            ...
            parent_frame.children = parent_frame.children + [
                child_frame,
            ]
...
def build_chain_from_mjcf(data, body: Union[None, str, int] = None):
    ...
    root_frame = frame.Frame(
        root_body.name,
        link=frame.Link(root_body.name, offset=tf.Transform3d(rot=root_body.quat, pos=root_body.pos)),
        joint=frame.Joint(),
    )
